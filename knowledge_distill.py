...
def get_args():
    ...
    ## style consistency
    parser.add_argument('--sc_weight', type=float, default=3.0,
...
class Trainer:
    ...
    def adv_training(self, input, label, criterion, epoch):
        ...
        mu_max, sig_max = mu.detach().clone(), sig.detach().clone()

        mu_max.requires_grad_(True)
        sig_max.requires_grad_(True)
        optimizer_max = torch.optim.SGD(params=[mu_max, sig_max], lr=self.args.adv_lr, momentum=0, weight_decay=0)  # default 20000

        images_source_max = input_normed * sig_max + mu_max

        self.model.eval()
        output = self.model(images_source_max, label, False, epoch)['logits']
        # output = self.model(images_source_max, label, True, epoch)
        ...
        input_max = input_normed * sig_max + mu_max

        input_max = input_max.clone().detach()
    ...
    def _do_epoch(self, epoch=None):
        ...
        for it, ((data, jig_l, class_l), d_idx) in enumerate(self.source_loader):
            ...
            # getting x,y,x_adv ready
            x = data.clone()
            y = class_l.clone()
            x_adv = self.adv_training(x, y, criterion, epoch)
            self.optimizer.zero_grad()
            self.model.zero_grad()
            
            # combine and get data to feed into student & teacher model
            data_for_stu = torch.cat((x_adv , x))
            data_for_t = torch.cat((x , x_adv))
            class_l = torch.cat((y,y)) 

            # get class prediction from student & teacher model
            class_logit_stu = self.model(data_for_stu)['logits']
            class_logit_t = self.teacher_model(data_for_t)['logits']

            # KL div between [model_t(x) and model_stu(x_adv)]+ KL div between [model_t(x_adv) and model_stu(x)]
            loss_con_style = self.style_consistency(torch.cat((class_logit_t,class_logit_stu)))

            # prediction
            loss_pred = criterion(class_logit_stu,class_l)

            #final loss
            loss = loss_con_style + loss_pred
            ...
            mmt=0.999
            for param_stu, param_t in zip(self.model.parameters(), self.teacher_model.parameters()):
                param_t.data = param_t.data * mmt + param_stu.data * (1.0 - mmt)
