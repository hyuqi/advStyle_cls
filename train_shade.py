...
class Trainer:
    ...
    def _do_epoch(self, epoch=None):
        ...
        for it, ((data, jig_l, class_l), d_idx) in enumerate(self.source_loader):
            ...
            # print(x_new.shape)
            scores = model(x_new)['logits']
            # print(scores.shape)
            loss = F.cross_entropy(scores, y)
            loss.backward()

            # update by gradient ascent, this is to "add noise"
            with torch.no_grad():
              style_feature_mu += lr_f * style_feature_mu.grad
              style_feature_sig += lr_f * style_feature_sig.grad
            ...
            loss_x = F.cross_entropy(score_x,y)

            # predict using classification model on adversial image and get loss
            score_x_adv = model(x_adv, class_l, not self.args.norsc, epoch)['logits']

            loss_x_adv = F.cross_entropy(score_x_adv,y)
